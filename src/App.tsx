...
export function App() {
  ...
  const onClickPlus = async (item: any) => {
    try {
  ...
  const onClickFav = async (item: any) => {
    try {
  ...
  const onRemoveCartItem = async (id: any) => {
    try {
  ...
  const isItemAdded = (parentID: any) => {
    return cartItems.some(
      (cartItem) => Number(cartItem.parentID) === Number(parentID)
    );
  };

  const isItemFavorited = (parentID: any) => {
  ...
  const setSearchInput = (event: any) => {
